...
pub fn compile(source: &str, vm: &mut Gc) -> Result<GcRef<Function>> {
    ...
    let mut parser = OldParser::new(scanner, vm);
...
impl<'source> OldParser<'source> {
    fn new(scanner: Scanner<'source>, gc: &'source mut Gc) -> OldParser<'source> {
    ...
    fn node_statement(&mut self, node_id: Token) {
        dbg!(node_id.lexeme);
        self.advance();
    }
    ...
    fn push_compiler(&mut self, function_type: FunctionType) {
        let function_name = self.gc.intern(self.previous.lexeme);
...
type ParseFn<'a> = fn(&mut OldParser<'a>, can_assign: bool) -> ();
...
impl<'source> ParseRuleTable<'source> {
    ...
    fn make_rule(token_type: TokenType) -> ParseRule<'source> {
        ...
        use TokenType::*;
        match token_type {
            LeftParen =>    ParseRule::new(Some(OldParser::grouping), Some(OldParser::call),   P::Call),
            ...
            Plus =>         ParseRule::new(None,                   Some(OldParser::binary), P::Term),
            Semicolon =>    ParseRule::new(None,                   None,                 P::None),
            Slash =>        ParseRule::new(None,                   Some(OldParser::binary), P::Factor),
            Star =>         ParseRule::new(None,                   Some(OldParser::binary), P::Factor),
            Bang =>         ParseRule::new(Some(OldParser::unary),    None,                 P::None),
            BangEqual =>    ParseRule::new(None,                   Some(OldParser::binary), P::Equality),
            Equal =>        ParseRule::new(None,                   None,                 P::None),
            EqualEqual =>   ParseRule::new(None,                   Some(OldParser::binary), P::Equality),
            Greater =>      ParseRule::new(None,                   Some(OldParser::binary), P::Comparison),
            GreaterEqual => ParseRule::new(None,                   Some(OldParser::binary), P::Comparison),
            Less =>         ParseRule::new(None,                   Some(OldParser::binary), P::Comparison),
            LessEqual =>    ParseRule::new(None,                   Some(OldParser::binary), P::Comparison),
            Identifier =>   ParseRule::new(Some(OldParser::variable), None,                 P::None),
            String =>       ParseRule::new(Some(OldParser::string),   None,                 P::None),
            Number =>       ParseRule::new(Some(OldParser::number),   None,                 P::None),
            And =>          ParseRule::new(None,                   Some(OldParser::and),    P::And),
            Class =>        ParseRule::new(None,                   None,                 P::None),
            Else =>         ParseRule::new(None,                   None,                 P::None),
            False =>        ParseRule::new(Some(OldParser::literal),  None,                 P::None),
            For =>          ParseRule::new(None,                   None,                 P::None),
            Fun =>          ParseRule::new(None,                   None,                 P::None),
            If =>           ParseRule::new(None,                   None,                 P::None),
            Nil =>          ParseRule::new(Some(OldParser::literal),  None,                 P::None),
            Or =>           ParseRule::new(None,                   Some(OldParser::or),     P::Or),
            Print =>        ParseRule::new(None,                   None,                 P::None),
            Return =>       ParseRule::new(None,                   None,                 P::None),
            Super =>        ParseRule::new(Some(OldParser::super_),   None,                 P::None),
            This =>         ParseRule::new(Some(OldParser::this),     None,                 P::None),
            True =>         ParseRule::new(Some(OldParser::literal),  None,                 P::None),
            Var =>          ParseRule::new(None,                   None,                 P::None),
            While =>        ParseRule::new(None,                   None,                 P::None),
            Digraph =>      ParseRule::new(None,                   None,                 P::None),
            Arrow =>        ParseRule::new(None,                   None,                 P::None),
